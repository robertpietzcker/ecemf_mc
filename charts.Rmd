...
# df_onlyModel <- calc_addVariable(df_onlyModel, "`Capacity|Electricity|Solar`" = "`Capacity|Electricity|Solar|PV`", units = "GW", completeMissing = TRUE) # not needed anymore with August upload
df <- rbind(df_woModel, df_onlyModel)
...
```{r ghg_emissions_line_plots, echo=FALSE, include=FALSE}
# paper plot
  plotData <- df %>%
    filter(variable == "Emi|Kyoto Gases|synthetic",
           period >=2005, period <= 2050,
...
```{r ghg_emissions_reductions_vs_1990_range1, echo=FALSE, include=FALSE}
...
#  mutate(Median = - 0.93 * value) # XXX CORRECT
  mutate(Median = - value)
#   summarise_at(vars(value), list(
#    Median = value))
...
# only historic: 
  ...
  g[["GHG_Emissions_synth_Reductions_vs_1990_range1_onlyHist"]] <- 
  ggplot(plotData,aes(x=period,y=Median)) +
  geom_hline(yintercept=0, color = "black", linewidth=1, alpha = 0.5) +
  geom_vline(xintercept=2020, color = "black", linewidth=1, alpha = 0.5,linetype="dashed") +
  geom_line(linewidth=1,color="black",alpha = 0) +
  geom_ribbon(aes(ymax = Q3, ymin = Q1), alpha = 0, fill = "#545454") +
  #    geom_line(aes(y=Min),linewidth=1,color="#545454",linetype = "dashed", alpha = 0.7) +
  #    geom_line(aes(y=Max),linewidth=1,color="#545454",linetype = "dashed", alpha = 0.7) +
  theme_minimal(base_size = 24) +
  #  scale_y_continuous(breaks = c(0,-0.2,-0.4,-0.6,-0.8,-1),labels = scales::percent_format(accuracy = 1)) +
  ...
  ggsave(paste0("./output/", time, "/svg/GHG_Emissions_synth_Reductions_vs_1990_range_white_onlyHist.svg"), g[["GHG_Emissions_synth_Reductions_vs_1990_range1_onlyHist"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
ggsave(paste0("./output/", time, "/png/GHG_Emissions_synth_Reductions_vs_1990_range_white_onlyHist.png"), g[["GHG_Emissions_synth_Reductions_vs_1990_range1_onlyHist"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
...
```{r land_use_and_geological_storage, echo=FALSE, include=FALSE}
...
# paper plot
  plotData <- df %>%
    filter(variable == "Emissions|CO2|Energy|Supply|Electricity", 
           model %in% modelsElecall, period %in% seq(2005,2050,5), scenario %in% c("WP1 NetZero"), region == "EU27 & UK (*)")

  plotDataRange <- plotData %>%
    group_by(period) %>%
    summarise_at(vars(value), list(
      Min = min,
      Mean = mean,
  ...
  g[["CO2_electricitySupply"]] <- ggplot(plotData,aes(x=period,y=value)) +
      ...
      geom_vline(xintercept=2020, color = "black", linewidth=1, alpha = 0.5,linetype="dashed") +
      geom_line(size=1,aes(color=model,linetype=model)) +
#      geom_line(data=plotDataRange,aes(x=period,y=Median),linewidth=1,color="black") +
#      geom_ribbon(data=plotDataRange,aes(x = period, ymax = Q3, ymin = Q1), alpha = 0.5, fill = "#545454") +    
#      geom_point(aes(color=model)) +
      geom_line(data=plotDataHistOrigN, size=2,linetype="solid", color = "black") +
#      geom_line(data=plotDataHistAv, size=2,linetype="solid", color = "black") +
#      geom_point(data=plotDataHistAv, shape=17 , size=7, color = "black") +
      theme_minimal(base_size = 24) +
      scale_color_manual(values = setNames(color[levels(plotData$model)],levels(plotData$model))) +
...
### Power sector emissions - rangeMix plot

```{r power_sector_co2_emi_rangeMix, echo=FALSE, include=FALSE}
...
#    scale_y_continuous(n.breaks = 8,labels = scales::percent_format(accuracy = 1)) +  
    ...
    theme(legend.position="none",
          ...
          plot.background = element_rect(fill="#FFFFFF", color = NA)) +
    geom_line(data=plotDataHistOrigNsum, aes(y=Median), size=3,linetype="solid", color = "black") +
        geom_line(data = plotDataSingle, aes(x=period, y = value,linetype=model),size = 0.5,color="blue") +
 #   scale_color_manual(values = setNames(color[levels(plotData$model)],levels(plotData$model))) + 
#    scale_linetype_manual(values = modelLinetype) +
  ...
  ggsave(paste0("./output/", time, "/svg/Elec_CO2_Emi_rangeMix.svg"), g[["Elec_CO2_Emi_rangeMix"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
  ggsave(paste0("./output/", time, "/png/Elec_CO2_Emi_rangeMix.png"), g[["Elec_CO2_Emi_rangeMix"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
...
#    scale_y_continuous(n.breaks = 8,labels = scales::percent_format(accuracy = 1)) +  
  ...
  ggsave(paste0("./output/", time, "/svg/Elec_CO2_Emi_range_onlyHist.svg"), g[["Elec_CO2_Emi_range_onlyHist"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
  ggsave(paste0("./output/", time, "/png/Elec_CO2_Emi_range_onlyHist.png"), g[["Elec_CO2_Emi_range_onlyHist"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
...
# paper plot
  ...
  ggsave(paste0("./output/", time, "/png/paper_CarbonCapture_line.png"), g[["CC_line"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
    quantile(filter(plotData, period == 2030)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles    
    quantile(filter(plotData, period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles     
    quantile(filter(plotData, period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles 
```
...
```{r Carbon_Capture_line_plot, echo=FALSE, include=FALSE}
...
```{r, echo=FALSE, fig.width=12, fig.height=8}
  g[["CarbonPrice_line"]]
```
## Carbon price range plot
```{r CarbonPrice_range, echo=FALSE, include=FALSE}
# paper plot
  plotData <- df %>%
    filter(variable == "Price|Carbon",    #"Price|Carbon (weighted by Final Energy)
           period >=2015, period <= 2050, scenario %in% c("WP1 NetZero"), model %in% modelsPowerEmi2040, region == "EU27 & UK (*)") %>%
    group_by(period) %>%
    summarise_at(vars(value), list(
      Min = min,
      Mean = mean,
...
#    geom_line(aes(x=period,y=Max),linewidth=0.5,color="black") +
#    geom_line(aes(x=period,y=Min),linewidth=0.5,color="black") +
    geom_ribbon(aes(ymax = Q3, ymin = Q1), alpha = 0.5, fill = "#545454") +
    theme_minimal(base_size = 24) +
#    scale_y_continuous(n.breaks = 8,labels = scales::percent_format(accuracy = 1)) +  
    scale_x_continuous(n.breaks = 8) + 
  ...
  ggsave(paste0("./output/", time, "/png/CarbonPrice_range.png"), g[["CarbonPrice_range"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
```
```{r, echo=FALSE, fig.width=12, fig.height=8}
  g[["CarbonPrice_range"]]
```
...
vars <- c(
  "Other"="Final Energy|Other Sector",
  ...
  "Total FE"    = "Final Energy")

pointVars <- c("Final Energy")
...
carrier_color <- c(  
  ...
  plotDataPrep <- df %>%
    filter(variable %in% vars, period %in% c(2020,2030,2040,2050), scenario == "WP1 NetZero", model %in% modelsFullSystem, region == "EU27 & UK (*)") 

  plotData <- plotDataPrep %>%
    filter(!variable %in% pointVars) %>%
    mutate(value = value * 1, # EJ 2 TWh
		   model = factor(model, levels = modelsFullSystem),
           variable = factor(variable, levels = vars)) #order
  
  plotDataPoints <- plotDataPrep %>%
    filter(variable %in% pointVars) %>%	   
    mutate(model = factor(model, levels = modelsFullSystem)) #order
  ...
  g[["FEsector_NZero"]] <- ggplot(plotData,aes(x=model,y=value,fill=variable)) +
    ...
    theme_minimal(base_size = 24) +
    scale_fill_manual(values = setNames(carrier_color[names(vars)],vars), labels = setNames(names(vars), vars)) +
    geom_point(data = plotDataPoints, aes(x=model, y = value), colour = "black", size = 4, shape = 18) + 
...
## Final Energy Carrier Stacked Bars
...
carrier_color <- c(  
  ...
  g[["FE_carrier_stackedBar"]] <- ggplot(plotData,aes(x=model,y=value,fill=variable)) +
    geom_hline(yintercept=0, color = "black", linewidth=1, alpha = 0.5) +
    geom_bar(stat='identity', alpha = 0.8) +
    facet_wrap(~period,nrow=1) +
    theme_minimal(base_size = 24) +
  ...
  ggsave(paste0("./output/", time, "/png/FE_carrier_stackedBar.png"), g[["FE_carrier_stackedBar"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
  
```
```{r, echo=FALSE, fig.width=12, fig.height=8}
  g[["FE_carrier_stackedBar"]]
```



## Final Energy line plot
```{r FE_line, echo=FALSE, include=FALSE}
  plotData <- df %>%
  ...
  g[["FE_line"]] <- ggplot(plotData,aes(x=period,y=value)) +
      geom_hline(yintercept=0, color = "black", linewidth=1, alpha = 0.5) +
...
#     scale_linetype_manual(values = c("historical"="solid","WP1 NPI"="solid", "WP1 NetZero"="solid")) +
  ...
  ggsave(paste0("./output/", time, "/png/FE_line.png"), g[["FE_line"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
    quantile(filter(plotData, period == 2030)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles    
    quantile(filter(plotData, period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles     
    quantile(filter(plotData, period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles 
```
...
## Final Energy Elec & H2 share total woBunker wo NonEnergy
```{r Final_Energy_Elec_and_H2_Share_Total_woBunkwoNonEn, echo=FALSE, include=FALSE}
# paper plot

  plotData <- df %>%
    filter(variable == "Final Energy|Electricity Share woBunkers woNonEnergy",
           period %in% seq(2005,2050,5), model %in% modelsFullSystem, scenario == "WP1 NetZero", region == "EU27 & UK (*)" )
 ...
 #   summarise_at(vars(value), list(
  ...
  g[["Final_Energy_Elec_and_H2_Share_Total_woBunkwoNonEn"]] <- 
    ggplot(plotData,aes(x=period,y=value)) +
    geom_hline(yintercept=0, color = "black", linewidth=1, alpha = 0.5) +
    geom_vline(xintercept=2020, color = "black", linewidth=1, alpha = 0.5,linetype="dashed") +
    geom_line(size=1,aes(color=model,linetype="dashed")) +
    geom_line(data = plotData2,aes(y=value, color=model,linetype=FELinetype[1]), size=1) +
#    geom_line(data= plotDataHistOrigN, size=1,linetype="solid", color = "black") +
    theme_minimal(base_size = 24) +
    scale_color_manual(values = setNames(color[levels(plotData$model)],levels(plotData$model))) +    
    ...
    theme(legend.position="top",
          panel.background = element_rect(fill="#FFFFFF", color = NA),
          plot.background = element_rect(fill="#FFFFFF", color = NA)) +
    labs(title="Share in Final Energy (w/o Bunk&nonEn)") +
    ylab(expression(paste("Share in Final Energy (%)"))) 
  

  ggsave(paste0("./output/", time, "/svg/paper_Final_Energy_Elec_and_H2_Share_Total_woBunkwoNonEn.svg"), g[["Final_Energy_Elec_and_H2_Share_Total_woBunkwoNonEn"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
  ggsave(paste0("./output/", time, "/png/paper_Final_Energy_Elec_and_H2_Share_Total_woBunkwoNonEn.png"), g[["Final_Energy_Elec_and_H2_Share_Total_woBunkwoNonEn"]] , device="png", width = 12, height = 8, dpi=100, units = "in")

    quantile(filter(plotData, period == 2020)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles 
    quantile(filter(plotData, period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles     
    quantile(filter(plotData, period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles 
    
    quantile(filter(plotData2, period == 2020)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles 
    quantile(filter(plotData2, period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles     
    quantile(filter(plotData2, period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles 
...
#    coord_cartesian(ylim = c(0, 120)) +
    ...
    geom_point(data = plotDataTotals, aes(x=model, y = value), 
                colour = "black", size = 4, shape = 18) + 
    scale_fill_manual(values = setNames(carrier_color[names(vars)],vars), labels = setNames(names(vars), vars)) +
    ggtitle("Carbon-containing final energy") +
    ylab("Carbon-containing final energy (EJ/yr)") +
    theme(axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0.5),
          axis.title.x=element_blank(),
          panel.background = element_rect(fill="#FFFFFF", color = NA),
          plot.background = element_rect(fill="#FFFFFF", color = NA))

  ggsave(paste0("./output/", time, "/png/paper_FE_GasLiqSol_Mix_Origin.png"), g[["FE_GasLiqSol_Mix_Origin"]] , device="png", width = 9, height = 8, dpi=100, units = "in")
  
  checkBioShares <- filter(df,scenario == "WP1 NetZero", variable == "FE|SolLiqGas|Bio Share", model %in% modelsFullSystem)
  pivot_periods(checkBioShares)
  checkFossShares <- filter(df,scenario == "WP1 NetZero", variable == "FE|SolLiqGas|Fossil Share", model %in% modelsFullSystem)
  pivot_periods(checkFossShares)
  
  checkCarbFe <- filter(df,scenario == "WP1 NetZero", variable == "FE|SolLiqGas", model %in% modelsFullSystem)
  pivot_periods(checkCarbFe)
  quantile(filter(checkCarbFe, period == 2020)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles
  quantile(filter(checkCarbFe, period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles
  quantile(filter(checkCarbFe, period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles
  
  checkFossUse <- filter(df,scenario == "WP1 NetZero", variable == "FE|SolLiqGas|Fossil", model %in% modelsFullSystem)
  pivot_periods(checkFossUse)
  quantile(filter(checkFossUse, period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles
  quantile(filter(checkFossUse, period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles
    
  checkCarbonFEUseReduction <- filter(df,scenario == "WP1 NetZero", variable == "FE|SolLiqGas reduction vs 2020", !model %in% c("Euro-Calliope"))
  pivot_periods(checkCarbonFEUseReduction)
  quantile(filter(checkCarbonFEUseReduction, period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles
  quantile(filter(checkCarbonFEUseReduction, period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1)) # give out interquartiles
...
carrier_color <- c(  
  ...
  plotData <- df %>%
   ...
   #        model = factor(model, levels = c("LIMES","IMAGE","MEESA","PRIMES","PROMETHEUS","REMIND","TIAM-ECN","WITCH")),
           variable = factor(variable, levels = vars)) #order
...
carrier_color <- c(  
  ...
  plotData <- df %>%
    filter(variable %in% vars,
           period %in% c(2020,2030,2040,2050), scenario == "WP1 NetZero", model %in% modelsElecall, region == "EU27 & UK (*)") %>%
  ...
  ggsave(paste0("./output/", time, "/svg/paperSI_Electricty_NZero.svg"), g[["Electricty_NZero"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
  ggsave(paste0("./output/", time, "/png/paperSI_Electricty_NZero.png"), g[["Electricty_NZero"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
...
carrier_color <- c(  
  ...
  plotData <- df %>%
    filter(variable %in% vars,
           period %in% c(2020,2030,2040,2050), model %in% modelsElecall, scenario == "WP1 NetZero", region == "EU27 & UK (*)") %>%
  ...
  ggsave(paste0("./output/", time, "/svg/paperSI_Elec_Capacity_NZero.svg"), g[["Elec_Capacity_NZero"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
  ggsave(paste0("./output/", time, "/png/paperSI_Elec_Capacity_NZero.png"), g[["Elec_Capacity_NZero"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
...
#      scale_linetype_manual(values = c("historical"="solid","WP1 NPI"="solid", "WP1 NetZero"="solid", "WP1 NetZero-LimBio"="dotted", "WP1 NetZero-LimCCS"="dotdash", "WP1 NetZero-LimNuc"="longdash")) +
  ...
  ggsave(paste0("./output/", time, "/svg/paper_SE_Electricity_VRE_Share.svg"), g[["SE_Electricity_VRE_Share"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
  ggsave(paste0("./output/", time, "/png/paper_SE_Electricity_VRE_Share.png"), g[["SE_Electricity_VRE_Share"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
...
```{r vre_share, echo=FALSE, include=FALSE}
...
#      scale_linetype_manual(values = c("historical"="solid","WP1 NPI"="solid", "WP1 NetZero"="solid", "WP1 NetZero-LimBio"="dotted", "WP1 NetZero-LimCCS"="dotdash", "WP1 NetZero-LimNuc"="longdash")) +
  ...
  ggsave(paste0("./output/", time, "/svg/paper_SE_Electricity_VRE.svg"), g[["SE_Electricity_VRE"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
  ggsave(paste0("./output/", time, "/png/paper_SE_Electricity_VRE.png"), g[["SE_Electricity_VRE"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
...
```{r SE_Electricity_bio_hydro, echo=FALSE, include=FALSE}
 ...
 # paper calculation
  ...
  auxData <- df %>%
    filter(variable %in% c("Secondary Energy|Electricity|Hydro","Secondary Energy|Electricity|Biomass","Secondary Energy|Electricity|Nuclear"),
           period >=2005, period <= 2050,
           scenario %in% c("WP1 NetZero"),
           region == "EU27 & UK (*)")
  ...
  auxData <- left_join(
   ...
   auxDataonlyPercentage <- select(auxData, model,variable,period,percentage) %>%
     mutate(value = percentage) %>% 
     select(-percentage)
    ...
    quantile(filter(auxData, variable == "Secondary Energy|Electricity|Hydro", period == 2050)[["percentage"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles
    ...
    quantile(filter(auxData, variable == "Secondary Energy|Electricity|Biomass", period == 2050)[["percentage"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out
    quantile(filter(auxData, variable == "Secondary Energy|Electricity|Nuclear", period == 2040)[["percentage"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles
    quantile(filter(auxData, variable == "Secondary Energy|Electricity|Nuclear", period == 2050)[["percentage"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles
   
 
    HydroElecIncreasevs2020 <- df %>%
    filter(variable %in% c("Secondary Energy|Electricity|Hydro"),
           period >=2005, period <= 2050, model %in% modelsElecall,
           scenario %in% c("WP1 NetZero"),
           region == "EU27 & UK (*)") %>% 
      mutate(value = value / 1.27)  # IEA = EMBER

    quantile(filter(HydroElecIncreasevs2020,period == 2020)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles    
    quantile(filter(HydroElecIncreasevs2020,period == 2030)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles    
    quantile(filter(HydroElecIncreasevs2020,period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles
    quantile(filter(HydroElecIncreasevs2020,period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles    
       
    BioElecIncreasevs2020 <- df %>%
    filter(variable %in% c("Secondary Energy|Electricity|Biomass"),
           period >=2005, period <= 2050, model %in% modelsElecall,
           scenario %in% c("WP1 NetZero"),
           region == "EU27 & UK (*)") %>% 
      mutate(value = value / 0.727)  # IEA has 0.821 for bioelectricity; Ember has 0.727

    quantile(filter(BioElecIncreasevs2020,period == 2030)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles    
    quantile(filter(BioElecIncreasevs2020,period == 2040)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles
    quantile(filter(BioElecIncreasevs2020,period == 2050)[["value"]], c(0, 0.25, 0.5, 0.75, 1))*100 # give out interquartiles

    NucElecIncreasevs2020 <- df %>%
    filter(variable %in% c("Secondary Energy|Electricity|Nuclear"),
           period >=2005, period <= 2050,
           scenario %in% c("WP1 NetZero"),
           region == "EU27 & UK (*)") %>% 
      mutate(value = value / 2.64)  # IEA-value for nuclear = Ember value
...
plotDataAllreorder <- plotDataAll %>%
  ...
  g[["Trade"]] <- ggplot(plotDataAllreorder,aes(x=model,y=value,fill=variable)) +
    ...
    scale_fill_manual(values = setNames(carrier_color[names(vars)],vars), labels = setNames(names(vars), vars)) +
    ggtitle("Net Imports") +
    ylab("Net Energy Imports (EJ/yr)") +
    theme(axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0.5),
          axis.title.x=element_blank(),
          panel.background = element_rect(fill="#FFFFFF", color = NA),
          plot.background = element_rect(fill="#FFFFFF", color = NA))

  ggsave(paste0("./output/", time, "/png/paper_Trade.png"), g[["Trade"]] , device="png", width = 9, height = 8, dpi=100, units = "in")
...
```{r Costs, echo=FALSE, include=FALSE}
