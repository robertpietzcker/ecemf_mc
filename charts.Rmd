...
## GHG Emissions reductions vs 2020 - range
...
```{r ghg_emissions_reductions_vs_2020_range, echo=FALSE, include=FALSE}
  ...
  g[["GHG_Emissions_Reductions_vs_2020_range"]] <- 
  ...
  ggsave(paste0("./output/", time, "/svg/GHG_Emissions_Reductions_vs_2020_range.svg"), g[["GHG_Emissions_Reductions_vs_2020_range"]] , device="svg", width = 12, height = 8, dpi=100, units = "in")
  ggsave(paste0("./output/", time, "/png/GHG_Emissions_Reductions_vs_2020_range.png"), g[["GHG_Emissions_Reductions_vs_2020_range"]] , device="png", width = 12, height = 8, dpi=100, units = "in")
...
df_typeA_orig <- calc_addVariable(df, "`Emi|Kyoto Gases|non-CO2 calc`" = "`Emissions|Kyoto Gases` - `Emissions|CO2`", units = "MtCO2e/yr", only.new = TRUE) %>%
  filter(period %in% c(2020,2030,2040,2050),
         scenario == "WP1 NetZero",
         region == "EU27 & UK (*)",
         model %in% model_w_nonCO2) 

df_nonCO2_adj <- left_join( # calculating adjustment factors based on 2020 historical values
  df_typeA_orig,
  df_typeA_orig %>%
    filter(period == 2020) %>%
    mutate(hist2020 = hist_nonCO2,
           multFactor = hist_nonCO2 / value,
           diffFactor = hist_nonCO2 - value) %>%
    select(-period, -value),
  by = join_by(model == model, scenario == scenario, region == region, variable == variable, unit == unit)
  ) %>%
  mutate(
...
df_nonCO2_w_data <- df_nonCO2_adj %>% # calculating the adjusted data using the difference between historical values to translate the model values instead of using a multiplicative factor
  mutate(value = valueAdjDiff) %>%
  select(model, scenario, region, variable, unit, period, value)
